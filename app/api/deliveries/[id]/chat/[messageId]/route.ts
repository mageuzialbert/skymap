...
import { authorizeRideParticipant } from '@/lib/chat-shared';

const TABLE = 'chat_messages';

async function loadMessage(messageId: string, deliveryId: string) {
  const { data } = await supabaseAdmin
    .from(TABLE)
    .select('id, delivery_id, sender_id, deleted_at, deleted_for')
    .eq('id', messageId)
...
export async function PATCH(
  ...
  const { ok } = await authorizeRideParticipant(params.id, user.id, role);
  if (!ok) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });

  const msg = await loadMessage(params.messageId, params.id);
